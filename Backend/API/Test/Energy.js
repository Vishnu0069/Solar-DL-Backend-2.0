...
const topenergyCollectionName = process.env.MONGODB_PLANT_ENERGY_NAME;
...
async function calculateAndStoreTopPerformingPlants() {
  ...
  try {
    ...
    const topPerformingCollection = database.collection(topenergyCollectionName);
    ...
    plants.forEach(plant => {
      const { Plantid, Date, EnergyOutput, Header } = plant;
      if (!Plantid || !Date) {
    ...
    console.log('Energy of the  plants have been calculated and stored.');
  } catch (err) {
