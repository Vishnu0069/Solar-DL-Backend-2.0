...
router.post("/verify_individual_user", auth, async (req, res) => {
  const { entityid, useremail } = req.body;

  if (!entityid || !useremail) {
    return res
      .status(400)
      .json({ message: "entityid and useremail parameters are required" });
  }
  ...
  try {
    ...
    if (userRows.length === 0) {
      ...
      return res
        .status(400)
        .json({ message: "Email not found, go ahead with verification" });
    }
