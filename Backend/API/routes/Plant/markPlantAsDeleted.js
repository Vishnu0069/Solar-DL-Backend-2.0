...
router.post('/disable', async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Plant not found or already marked as deleted' });
    }

    res.status(200).json({ message: 'Plant marked as deleted successfully' });
  } catch (error) {
    console.error('Error marking plant as deleted:', error);
    res.status(500).json({ message: 'Error marking plant as deleted', error: error.message });
  }
});
