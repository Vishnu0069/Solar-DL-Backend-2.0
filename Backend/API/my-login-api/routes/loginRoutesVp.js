...
// Written on Date 26-04-2024const express = require('express');
...
router.post('/login', async (req, res) => {
  ...
  const queryUser = 'SELECT Userid, passwordhashcode FROM UserMaster WHERE User_Email = ?';
  connection.query(queryUser, [email], async (err, users) => {
    ...
    const queryUserRoles = 'SELECT usertype, role, plantid, integratorid FROM user_roles WHERE userid = ?';
    connection.query(queryUserRoles, [user.Userid], (err, roles) => {
      if (err) {
