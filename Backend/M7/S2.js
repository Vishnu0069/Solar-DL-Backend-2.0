...
async function main() {
  try {
    ...
    // Written by Vishnu Prasad S
    // Written at date: 20-04-2024
    const cursor = deviceDataCollection.find({
    ...
    for (const [plantId, data] of Object.entries(plantOutputs)) {
      ...
      if (updateResult.upsertedCount > 0 || updateResult.modifiedCount > 0) {
        await logToFile(
          ...
          "S2 Service",
          "success",
          `Updated/Inserted data for plant ${plantId}`
        );
      } else {
        await logToFile(
          "M7(S2)",
          ...
          "S2 Service",
          "NoChange",
          `No changes made for plant ${plantId}`
        );
      }
    }
    ...
    logToFile(
      ...
      "S2 Service",
      "success",
      "Data processed and inserted/updated successfully."
    );
  } catch (err) {
